...
function check2(){
  ...
  // weeks.length == 7
  // weeks 의 마지막 index 번호 == 6 == weeks -1
...
/** 배열 X for 문 확인 3*/
/* 
  prompt를 이용해서 숫자 5개를 입력받아 배열에 저장 후 저장된 갑, 합계, 평균을 console에 출력
...
function check4(){
  let arr = new Array(5); // 5칸 짜리 배열 생성

  let sum = 0; //합계 저장용 변수

  for(let i = 0; i < arr.length; i++){
    // 입력 받자마자 Number 타입으로 변환하여 배열요소에 저장
    arr[i] = Number(prompt(`${i}번째에 저장할 숫자입력`))
...
function check5(){
  const leng = Number(prompt("생성할 난수 범위지정"));
...
function check6(){
  ...
  lotto.sort(function(a,b){return a-b;}); //배열 내 숫자 정렬방법
...
function selectMenu(){
  // 점심메뉴로 초기화 된 배열 생성
  const menus = ["라면+김밥", "제육볶음", "치즈돈까스", "샐러드", 
                 "맘스터치", "알탕", "돼지국밥", "닭갈비", "샌드위치", "KFC", "냉면", "콩국수", "쭈꾸미",
                 "초밥", "쌀국수", "우육탕면", "김치찜", "고등어구이"]; //18개
  // menus 배열에서 랜덤으로 하나 뽑아서 menuResult에 출력하기

  const leng = Number(menus.length);
  const randomNumber = Math.floor(Math.random()*leng);
...
function check7(){
  ...
  console.log("2차원 배열 arr의 길이(행의 갯수) : ", arr.length);
